...
void main() {
  group('BuildOrderComputer', () {
    ...
    test(
      'BO-COMP-1: Projects with no deps sorted alphabetically [2026-02-20]',
      () {
    ...
    test('BO-COMP-4: Circular dependency returns null [2026-02-20]', () {
      ...
      final paths = ['${tempDir.path}/cycle_a', '${tempDir.path}/cycle_b'];
    ...
    test(
      ...
      () {
        ...
        final paths = ['${tempDir.path}/proj_b', '${tempDir.path}/proj_a'];
    ...
    test('BO-COMP-5: External deps are ignored [2026-02-20]', () {
      ...
      final paths = ['${tempDir.path}/my_app', '${tempDir.path}/my_lib'];
    ...
    test('BO-COMP-6: Dev dependencies included when flag set [2026-02-20]', () {
      ...
      final paths = ['${tempDir.path}/lib_b', '${tempDir.path}/lib_a'];
      ...
      // With dev deps
      final resultWithDev = BuildOrderComputer.computeBuildOrder(
        paths,
        includeDev: true,
      );
      final namesWithDev = resultWithDev!
          .map((p) => p.split('/').last)
          .toList();
  ...
  group('FolderSorter.sortByBuildOrder', () {
    test('BO-SORT-1: Sorts items by global order [2026-02-20]', () {
      ...
      final result = sorter.sortByBuildOrder(items, (s) => s, globalOrder);
    ...
    test(
      'BO-SORT-3: Empty global order returns input unchanged [2026-02-20]',
      () {
        final sorter = FolderSorter();
        final items = ['/c', '/a', '/b'];
  ...
  group('BuildBase.traverse with buildOrder', () {
    ...
    setUp(() {
      ...
      File('${tempDir.path}/buildkit_master.yaml').writeAsStringSync('');

      // lib_core: no deps
      final core = Directory('${tempDir.path}/lib_core');
      core.createSync();
      File('${core.path}/pubspec.yaml').writeAsStringSync('name: lib_core\n');
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          reason: 'lib_core should be found',
        );
        expect(
          utilsIdx,
          greaterThanOrEqualTo(0),
          reason: 'lib_utils should be found',
        );
        ...
        expect(utilsIdx, lessThan(appIdx), reason: 'lib_utils before app');
      },
    );
