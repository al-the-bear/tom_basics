...
void main() {
  ...
  group('BuildBase.traverse with ProjectTraversalInfo', () {
    test(
      'BB-INT-1: Finds Dart projects with DartProjectFolder nature [2026-02-12]',
      () async {
        ...
        await BuildBase.traverse(
          ...
          run: (ctx) async {
            ...
            processedNatures[ctx.name] = ctx.natures
        ...
        expect(
          processedPaths.any((p) => p.contains('zom_test_flutter')),
          isTrue,
          reason: 'Should find zom_test_flutter',
        );
        expect(
          processedPaths.any((p) => p.contains('zom_test_package')),
          isTrue,
          reason: 'Should find zom_test_package',
        );
        expect(
          processedPaths.any((p) => p.contains('zom_test_standalone')),
          isTrue,
        ...
        expect(
          processedPaths.any((p) => p.contains('zom_typescript')),
          isFalse,
          reason: 'Should NOT find TypeScript-only projects',
        );
        expect(
          processedPaths.any((p) => p.contains('zom_extension')),
          isFalse,
        ...
        if (processedNatures.containsKey('zom_test_flutter')) {
          expect(
            processedNatures['zom_test_flutter'],
            contains('FlutterProjectFolder'),
          );
        }
        if (processedNatures.containsKey('zom_test_package')) {
          expect(
            processedNatures['zom_test_package'],
            contains('DartPackageFolder'),
          );
        }
        if (processedNatures.containsKey('zom_test_standalone')) {
          expect(
            processedNatures['zom_test_standalone'],
    ...
    test(
      'BB-INT-2: Finds Flutter projects specifically [2026-02-12]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: zomTestRoot,
          scan: zomTestRoot,
    ...
    test(
      'BB-INT-5: Finds TypeScript projects with TypeScriptFolder nature [2026-02-12]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: zomTestRoot,
          scan: zomTestRoot,
    ...
    test(
      'BB-INT-6: Finds VS Code extensions with VsCodeExtensionFolder nature [2026-02-12]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: zomTestRoot,
          scan: zomTestRoot,
    ...
    test('BB-INT-7: Excludes test projects by default [2026-02-12]', () async {
      ...
      expect(
        foundProjects.any((n) => n.startsWith('zom_')),
        isFalse,
    ...
    test(
      'BB-INT-8: TestProjectsOnly filters to only zom_* projects [2026-02-12]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: zomTestRoot,
          scan: zomTestRoot,
    ...
    test(
      'BB-INT-9: TestProjectsOnly finds ALL test project types including TypeSc... [2026-02-12]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: zomTestRoot,
          scan: zomTestRoot,
        ...
        await BuildBase.traverse(
          ...
          run: (ctx) async {
            foundProjects.add(ctx.name);
            projectNatures[ctx.name] = ctx.natures
                .map((n) => n.runtimeType.toString())
                .toList();
            return true;
          },
        ...
        if (projectNatures.containsKey('zom_extension')) {
          expect(
            projectNatures['zom_extension'],
            contains('VsCodeExtensionFolder'),
    ...
    test(
      'BB-INT-10: IncludeTestProjects includes BOTH regular and test projects [2026-02-12]',
      () async {
        ...
        try {
          final info = ProjectTraversalInfo(
            ...
            recursive: true,
            includeTestProjects:
                true, // --test: include test projects IN ADDITION to regular
          );
          ...
          final regularProjects = foundProjects
              .where((n) => !n.startsWith('zom_'))
              .toList();
          expect(
            regularProjects,
            isNotEmpty,
          ...
          final testProjects = foundProjects
              .where((n) => n.startsWith('zom_'))
              .toList();
          expect(
            testProjects,
          ...
          expect(
            regularProjects.any((n) => n.contains('tom_')),
            isTrue,
  ...
  group('BuildBase.traverse with GitTraversalInfo', () {
    ...
    test(
      'BB-INT-12: InnerFirst mode processes inner repos before outer [2026-02-12]',
      () async {
        final info = GitTraversalInfo(
          executionRoot: workspaceRoot,
          gitMode: GitTraversalMode.innerFirst,
        ...
        if (order.length >= 2) {
          for (int i = 0; i < order.length - 1; i++) {
            ...
            expect(
              currentDepth,
              greaterThanOrEqualTo(nextDepth),
    ...
    test(
      'BB-INT-13: OuterFirst mode processes outer repos before inner [2026-02-12]',
      () async {
        final info = GitTraversalInfo(
          executionRoot: workspaceRoot,
  ...
  group('BuildBase.findProjects', () {
    test(
      'BB-INT-14: Returns CommandContext list without executing [2026-02-12]',
      () async {
    ...
    test('BB-INT-17: Respects requiredNatures filter [2026-02-12]', () async {
      ...
      for (final ctx in contexts) {
        expect(
          ctx.natures.any((n) => n is DartProjectFolder),
          isTrue,
  ...
  group('Workspace VS Code Extension', () {
    test(
      'BB-INT-21: Detects tom_vscode_extension as both VsCodeExtension and TypeS... [2026-02-12]',
      () async {
        final vsCodePath = p.join(
          workspaceRoot,
          'xternal',
          'tom_module_vscode',
        ...
        expect(
          ...
          isTrue,
          reason:
              'tom_vscode_extension should be detected as VsCodeExtensionFolder',
        );
        expect(
          natures.any((n) => n is TypeScriptFolder),
          isTrue,
          reason:
              'tom_vscode_extension should also be detected as TypeScriptFolder',
        );
      },
    );

    test(
      'BB-INT-22: Traverse finds VS Code extension with TypeScriptFolder nature [2026-02-12]',
      () async {
        final vsCodeModulePath = p.join(
          workspaceRoot,
          'xternal',
          'tom_module_vscode',
        );
        ...
        expect(
          tsProjects,
          contains('tom_vscode_extension'),
          reason:
              'Should find tom_vscode_extension with TypeScriptFolder nature',
        );

        // Verify it also has VsCodeExtensionFolder
        if (projectNatures.containsKey('tom_vscode_extension')) {
          expect(
            projectNatures['tom_vscode_extension'],
            contains('VsCodeExtensionFolder'),
            reason:
                'tom_vscode_extension should also have VsCodeExtensionFolder',
          );
        }
      },
    );
