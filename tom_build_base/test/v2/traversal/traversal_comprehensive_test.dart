...
void main() {
  ...
  group('Basic Project Traversal', () {
    test(
      'TRV-PROJ-1: Recursive scan finds all expected projects [2026-06-30]',
      () async {
    ...
    test(
      'TRV-PROJ-2: Non-recursive scan finds projects in containers but does not descend into projects [2026-07-05]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
        ...
        expect(
          ...
          contains('proj_delta'),
          reason:
              'Non-recursive should find proj_delta — it is a project in '
        ...
        expect(
          ...
          contains('module_one'),
          reason:
              'Non-recursive should find module_one — a project in the '
              'container xternal/',
        );
        ...
        expect(
          found.any((n) => n == 'pkg_one_a'),
          isFalse,
          reason:
              'Non-recursive should NOT descend into project module_one to '
              'find its nested project pkg_one_a',
        );
        expect(
          found.any((n) => n == 'pkg_two_a'),
          isFalse,
          reason:
              'Non-recursive should NOT descend into project module_two to '
    ...
    test(
      'TRV-PROJ-3: Scan with different root finds only that subtree [2026-06-30]',
      () async {
    ...
    test(
      'TRV-PROJ-4: ExecutionRoot != scan path produces correct relativePath [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
  ...
  group('Skip File Handling', () {
    test(
      'TRV-SKIP-1: buildkit_skip.yaml skips folder and descendants [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
        ...
        expect(
          found,
          isNot(contains('skipped_project')),
          reason: 'buildkit_skip.yaml should skip the folder',
        );
        expect(
          found,
          isNot(contains('nested_in_skip')),
    ...
    test(
      'TRV-SKIP-2: tom_skip.yaml (global skip) skips folder [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
    ...
    test(
      'TRV-SKIP-3: Workspace boundary (buildkit_master.yaml) stops recursion [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
    ...
    test(
      'TRV-SKIP-4: Root directory is never skipped even if it matches skip markers [2026-06-30]',
      () async {
  ...
  group('Git Traversal', () {
    test(
      'TRV-GIT-1: Git traversal finds all repos (root + submodules) [2026-06-30]',
      () async {
    ...
    test(
      'TRV-GIT-2: InnerFirst orders deeper repos before shallower [2026-06-30]',
      () async {
        final info = GitTraversalInfo(
          executionRoot: fixture.rootPath,
          gitMode: GitTraversalMode.innerFirst,
    ...
    test(
      'TRV-GIT-3: OuterFirst orders shallower repos before deeper [2026-06-30]',
      () async {
        final info = GitTraversalInfo(
          executionRoot: fixture.rootPath,
    ...
    test(
      'TRV-GIT-4: Git traversal detects GitFolder nature [2026-06-30]',
      () async {
        final info = GitTraversalInfo(
          executionRoot: fixture.rootPath,
          gitMode: GitTraversalMode.innerFirst,
        ...
        await BuildBase.traverse(
          ...
          requiredNatures: {FsFolder},
          run: (ctx) async {
            naturesPerRepo[ctx.name] = ctx.natures
        ...
        for (final entry in naturesPerRepo.entries) {
          expect(
            ...
            contains('GitFolder'),
            reason:
                '${entry.key} should have GitFolder nature in git traversal',
          );
        }
      },
    );
  ...
  group('Filter Pipeline', () {
    test(
      'TRV-FILT-1: Project pattern (-p) filters to matching names [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
    ...
    test(
      'TRV-FILT-2: Exclude pattern removes matching paths [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
    ...
    test(
      'TRV-FILT-3: ExcludeProjects removes by project name [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
    ...
    test(
      'TRV-FILT-4: Required natures filter to specific project types [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
    ...
    test(
      'TRV-FILT-5: Git modules filter limits to specific submodules [2026-06-30]',
      () async {
        final info = GitTraversalInfo(
          executionRoot: fixture.rootPath,
          gitMode: GitTraversalMode.innerFirst,
  ...
  group('Nature Detection', () {
    test(
      'TRV-NAT-1: Dart console app has DartConsoleFolder nature [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
        ...
        expect(
          natures.containsKey('proj_alpha'),
          isTrue,
    ...
    test(
      'TRV-NAT-2: Dart package has DartPackageFolder nature [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
    ...
    test(
      'TRV-NAT-3: TypeScript project has TypeScriptFolder nature [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
  ...
  group('CLI Arg Parsing', () {
    ...
    test(
      'TRV-CLI-13: toProjectTraversalInfo applies defaults correctly [2026-06-30]',
      () {
        final parser = CliArgParser();
    ...
    test(
      'TRV-CLI-14: toGitTraversalInfo with -i produces innerFirst [2026-06-30]',
      () {
        final parser = CliArgParser();
        final args = parser.parse(['-i']);
        final info = args.toGitTraversalInfo(executionRoot: fixture.rootPath);
    ...
    test(
      'TRV-CLI-15: toGitTraversalInfo without mode returns null [2026-06-30]',
      () {
        final parser = CliArgParser();
    ...
    test(
      'TRV-CLI-16: Default recursive is false (workspace mode) [2026-06-30]',
      () {
        final parser = CliArgParser();
        final args = parser.parse([]);
        final info = args.toProjectTraversalInfo(
  ...
  group('External Process Traversal', () {
    ...
    setUpAll(() {
      ...
      final buildBaseRoot = p.normalize(p.join(Directory.current.path));
      toolPath = p.join(
        buildBaseRoot,
        'test',
        'v2',
    ...
    test(
      'TRV-EXT-1: External tool scans recursively with -r -R [2026-06-30]',
      () async {
        ...
        final folders = (output['folders'] as List)
    ...
    test(
      'TRV-EXT-2: External tool passes -p project filter [2026-06-30]',
      () async {
        final output = await runTool([
          '-R',
          fixture.rootPath,
        ...
        expect(
          names,
          isNot(contains('proj_beta')),
          reason: 'Should be filtered by -p',
        );
      },
    );

    test('TRV-EXT-3: External tool passes -x exclude [2026-06-30]', () async {
      final output = await runTool([
        '-R',
        fixture.rootPath,
        '-s',
    ...
    test(
      'TRV-EXT-4: External tool git inner-first with -i [2026-06-30]',
      () async {
        final output = await runTool(['-R', fixture.rootPath, '-i']);
        ...
        final parsedArgs = output['parsedArgs'] as Map<String, dynamic>;
        expect(parsedArgs['innerFirstGit'], isTrue);
      },
    );

    test(
      'TRV-EXT-5: External tool git outer-first with -o [2026-06-30]',
      () async {
        final output = await runTool(['-R', fixture.rootPath, '-o']);
    ...
    test(
      'TRV-EXT-6: Parsed args accurately reflect CLI input [2026-06-30]',
      () async {
        final output = await runTool([
          '-R',
          fixture.rootPath,
        ...
        expect((parsedArgs['excludePatterns'] as List), contains('*skip*'));
      },
    );

    test(
      'TRV-EXT-7: External tool reports natures for each folder [2026-06-30]',
      () async {
        final output = await runTool([
          '-R',
          fixture.rootPath,
    ...
    test(
      'TRV-EXT-8: Positional args are captured correctly [2026-06-30]',
      () async {
        final output = await runTool([
          '-R',
          fixture.rootPath,
  ...
  group('Edge Cases', () {
    test(
      'TRV-EDGE-1: Empty scan directory returns empty results [2026-06-30]',
      () async {
    ...
    test(
      'TRV-EDGE-2: Non-existent scan path returns empty results [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
    ...
    test(
      'TRV-EDGE-3: Concurrent traversals use separate state [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
    ...
    test(
      'TRV-EDGE-4: Traversal with all filters returns subset [2026-06-30]',
      () async {
        final info = ProjectTraversalInfo(
          executionRoot: fixture.rootPath,
          scan: fixture.rootPath,
