...
void main() {
  ...
  group('NatureDetector', () {
    group('Flutter project detection', () {
      test(
        'BB-NAT-1: Detects FlutterProjectFolder for zom_test_flutter [2026-02-12]',
        () {
          ...
          expect(
            natures.any((n) => n is FlutterProjectFolder),
            isTrue,
            reason: 'Should detect FlutterProjectFolder',
          );
          expect(
            natures.any((n) => n is DartConsoleFolder),
            isFalse,
            reason: 'Should NOT be detected as console app',
          );
          expect(
            natures.any((n) => n is DartPackageFolder),
            isFalse,
      ...
      test(
        'BB-NAT-2: FlutterProjectFolder has correct metadata [2026-02-12]',
        () {
          final path = p.join(zomTestRoot, 'zom_test_flutter');
          final folder = FsFolder(path: path);
    ...
    group('Dart console app detection', () {
      test(
        'BB-NAT-3: Detects DartConsoleFolder for zom_test_standalone [2026-02-12]',
        () {
    ...
    group('Dart package detection', () {
      test(
        'BB-NAT-5: Detects DartPackageFolder for zom_test_package [2026-02-12]',
        () {
      ...
      test(
        'BB-NAT-7: DartPackageFolder requires lib/src/ per design spec [2026-02-12]',
        () {
          ...
          expect(
            ...
            isNotNull,
            reason:
                'Project with pubspec.yaml AND lib/src/ should be DartPackageFolder',
          );
          expect(
            pkg!.hasLibSrc,
            isTrue,
    ...
    group('VS Code extension detection', () {
      test(
        'BB-NAT-8: Detects VsCodeExtensionFolder for zom_extension [2026-02-12]',
        () {
          ...
          expect(
            natures.any((n) => n is VsCodeExtensionFolder),
            isTrue,
      ...
      test(
        'BB-NAT-9: VsCodeExtensionFolder has correct metadata [2026-02-12]',
        () {
          final path = p.join(zomTestRoot, 'zom_extension');
          final folder = FsFolder(path: path);
    ...
    group('TypeScript project detection', () {
      test(
        'BB-NAT-10: Detects TypeScriptFolder for zom_typescript [2026-02-12]',
        () {
          ...
          expect(
            natures.any((n) => n is TypeScriptFolder),
            isTrue,
      ...
      test(
        'BB-NAT-11: Zom_extension also has TypeScriptFolder nature [2026-02-12]',
        () {
    ...
    group('TomBuildFolder detection', () {
      test(
        'BB-NAT-14: Detects TomBuildFolder when tom_project.yaml exists [2026-02-12]',
        () {
          final path = p.join(zomTestRoot, 'zom_test_flutter');
          final folder = FsFolder(path: path);
          ...
          expect(
            natures.any((n) => n is TomBuildFolder),
            isTrue,
    ...
    group('Multiple natures', () {
      ...
      test(
        'BB-NAT-16: VS Code extension has multiple natures [2026-02-12]',
        () {
          final path = p.join(zomTestRoot, 'zom_extension');
          final folder = FsFolder(path: path);
    ...
    group('Nature type hierarchy', () {
      test(
        'BB-NAT-18: FlutterProjectFolder is a DartProjectFolder [2026-02-12]',
        () {
          final path = p.join(zomTestRoot, 'zom_test_flutter');
          final folder = FsFolder(path: path);
      ...
      test(
        'BB-NAT-19: DartConsoleFolder is a DartProjectFolder [2026-02-12]',
        () {
          final path = p.join(zomTestRoot, 'zom_test_standalone');
          final folder = FsFolder(path: path);
      ...
      test(
        'BB-NAT-20: DartPackageFolder is a DartProjectFolder [2026-02-12]',
        () {
          final path = p.join(zomTestRoot, 'zom_test_package');
          final folder = FsFolder(path: path);
  ...
  group('DartProjectFolder static methods', () {
    test(
      'BB-NAT-22: IsDartProject returns true for Dart projects [2026-02-12]',
      () {
        expect(
          DartProjectFolder.isDartProject(
            p.join(zomTestRoot, 'zom_test_flutter'),
          ),
          isTrue,
        );
        expect(
          DartProjectFolder.isDartProject(
            p.join(zomTestRoot, 'zom_test_package'),
          ),
          isTrue,
        );
        expect(
          DartProjectFolder.isDartProject(
            p.join(zomTestRoot, 'zom_test_standalone'),
          ),
          isTrue,
        );
      },
    ...
    test(
      'BB-NAT-23: IsDartProject returns false for non-Dart projects [2026-02-12]',
      () {
        expect(
          DartProjectFolder.isDartProject(
            p.join(zomTestRoot, 'zom_typescript'),
          ),
          isFalse,
        );
        expect(
          DartProjectFolder.isDartProject(p.join(zomTestRoot, 'zom_extension')),
          isFalse,
        );
      },
    );
  ...
  group('GitFolder static methods', () {
    ...
    test(
      'BB-NAT-25: IsGitFolder returns false for non-git folders [2026-02-12]',
      () {
  ...
  group('VsCodeExtensionFolder static methods', () {
    test(
      'BB-NAT-26: IsVsCodeExtension returns true for extensions [2026-02-12]',
      () {
        expect(
          VsCodeExtensionFolder.isVsCodeExtension(
            p.join(zomTestRoot, 'zom_extension'),
          ),
          isTrue,
        );
      },
    ...
    test(
      'BB-NAT-27: IsVsCodeExtension returns false for regular TS projects [2026-02-12]',
      () {
        expect(
          VsCodeExtensionFolder.isVsCodeExtension(
            p.join(zomTestRoot, 'zom_typescript'),
  ...
  group('TypeScriptFolder static methods', () {
    test(
      'BB-NAT-28: IsTypeScriptProject returns true for TS projects [2026-02-12]',
      () {
        expect(
          TypeScriptFolder.isTypeScriptProject(
            p.join(zomTestRoot, 'zom_typescript'),
          ),
          isTrue,
        );
    ...
    test(
      'BB-NAT-29: IsTypeScriptProject returns false for Dart projects [2026-02-12]',
      () {
        expect(
          TypeScriptFolder.isTypeScriptProject(
            p.join(zomTestRoot, 'zom_test_flutter'),
  ...
  group('Design specification: Detection Rules', () {
    ...
    test(
      'BB-NAT-30: GitFolder: .git/ directory or .git file exists [2026-02-12]',
      () {
        // .git directory case (workspace root is a normal git repo)
        expect(
          Directory(p.join(workspaceRoot, '.git')).existsSync() ||
    ...
    test('BB-NAT-31: DartProjectFolder: pubspec.yaml exists [2026-02-12]', () {
      ...
      expect(
        natures.any((n) => n is DartProjectFolder),
        isTrue,
    ...
    test(
      'BB-NAT-32: FlutterProjectFolder: pubspec.yaml contains sdk: flutter [2026-02-12]',
      () {
        final path = p.join(zomTestRoot, 'zom_test_flutter');
        ...
        expect(
          ...
          isTrue,
          reason:
              'Dart project with sdk: flutter should be FlutterProjectFolder',
        );
      },
    );

    test(
      'BB-NAT-33: DartConsoleFolder: pubspec.yaml exists AND bin/ exists [2026-02-12]',
      () {
        final path = p.join(zomTestRoot, 'zom_test_standalone');
        expect(
          File(p.join(path, 'pubspec.yaml')).existsSync(),
        ...
        expect(
          Directory(p.join(path, 'bin')).existsSync(),
          isTrue,
    ...
    test(
      'BB-NAT-34: DartPackageFolder: pubspec.yaml exists AND lib/src/ exists [2026-02-12]',
      () {
        final path = p.join(zomTestRoot, 'zom_test_package');
        expect(
          File(p.join(path, 'pubspec.yaml')).existsSync(),
        ...
        expect(
          Directory(p.join(path, 'lib', 'src')).existsSync(),
          isTrue,
    ...
    test(
      'BB-NAT-35: VsCodeExtensionFolder: package.json contains engines.vscode [2026-02-12]',
      () {
        final path = p.join(zomTestRoot, 'zom_extension');
        ...
        expect(
          packageJson.contains('"engines"') && packageJson.contains('"vscode"'),
          isTrue,
          reason:
              'Test precondition: should have engines.vscode in package.json',
        );
    ...
    test(
      'BB-NAT-38: BUG: Dart project without bin/ or lib/src/ should NOT be DartP... [2026-02-12]',
      () {
        // Design spec states:
...
''');
          ...
          expect(
            Directory(p.join(tempDir.path, 'bin')).existsSync(),
            isFalse,
            reason: 'Test precondition: should NOT have bin/',
          );
          expect(
            Directory(p.join(tempDir.path, 'lib', 'src')).existsSync(),
            isFalse,
          ...
          expect(
            ...
            isTrue,
            reason:
                'Should be detected as DartProjectFolder (pubspec.yaml exists)',
          );
          ...
          expect(
            ...
            isFalse,
            reason:
                'Per design spec: FlutterProjectFolder requires sdk: flutter',
          );
        } finally {
